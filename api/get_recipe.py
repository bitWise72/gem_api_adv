...
{
  "step 1": { "procedure": <string>, "measurements": [(ingredient1, measurement1, "nutritional_info": {
        "calories": <int>,  
        "protein": <float>,   # grams
        ...
        "fiber": <float>,  # grams
        "sugar": <float>,  # grams
        }), (ingredient2, measurement2, "nutritional_info": {
        "calories": <int>,  
        "protein": <float>,   # grams
        "carbohydrates": <float>,  # grams  
        "fat": <float>,  # grams
...
Make sure to include the nutritional information in the response, for each step add only the exact and accurate nutritionall value based on the quantities mentioned in the measurements for a given ingredient.
...
def get_gemini_response(prompt_text=None, client=None, image_file=None, image_url=None):
    ...
    if len(prompt_text.strip().split()) <= 2:
        prompt_text = f"Generate the recipe for {prompt_text.strip()} and provide accurate measurements in grams and time in minutes along with the procedure and nutritional information as asked in the system"
